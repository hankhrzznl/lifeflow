...
import { ArrowLeft, List, PieChart, Wallet, BookOpen, Settings } from "lucide-react";
...
const tabs: Tab[] = [
  { label: "明细", path: "/accounting", icon: List },
  { label: "图表", path: "/accounting/chart", icon: PieChart },
  { label: "资产", path: "/accounting/assets", icon: Wallet },
  { label: "账本", path: "/accounting/ledgers", icon: BookOpen },
  { label: "设置", path: "/accounting/settings", icon: Settings },
];
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/85 backdrop-blur-xl border-t border-black/5 pb-[max(8px,env(safe-area-inset-bottom))]">
      <div className="flex justify-around items-center h-14">
        {tabs.map((tab) => {
          const isActive = pathname === tab.path || (tab.path !== "/accounting" && pathname.startsWith(tab.path));
          return (
            <Link
              key={tab.path}
              href={tab.path}
            ...
            >
              {isActive && (
                <motion.span
                  layoutId="accounting-nav-indicator"
    ...
    <div className="min-h-screen bg-[var(--bg-secondary)]" style={{ paddingBottom: "calc(56px + max(8px, env(safe-area-inset-bottom)))" }}>
      ...
      <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-xl border-b border-black/5">
        <div className="flex items-center h-12 px-4">
          ...
          <h1 className="text-base font-semibold flex-1" style={{ color: BRAND }}>记账</h1>
        </div>
      </header>
