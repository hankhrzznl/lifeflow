...
export default function GrowthHubPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const list = await getSubmodulesByParent("growth");
      setSubmodules(list);
      setLoading(false);
    };
    load();
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
      <div className="mx-auto max-w-5xl px-5 pt-8 pb-24 md:px-8 md:pt-10">
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900">成长</h1>
          <p className="text-slate-400 mt-1">规划 · 成长 · 习惯</p>
        </div>
