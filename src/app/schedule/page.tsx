...
import {
  ...
  X, CheckCircle, Circle, AlertCircle,
} from "lucide-react";
import {
  ...
  generateDaySchedule, updateDaySchedule,
} from "@/lib/db";
...
function TodayView() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const ds = await generateDaySchedule(today);
      setSchedule(ds);
      if (ds) {
        const tmpl = await import("@/lib/db").then((m) => {
          return m.getAllTemplates().then((list) => list.find((t) => t.id === ds.templateId) ?? null);
        });
        setTemplate(tmpl);
        // Check if last day
        if (tmpl) {
  ...
  if (noTemplate) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-sm text-gray-500 mb-1">今天没有生效的模板</p>
    ...
    <div>
      ...
      {!schedule || schedule.events.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-500">今日日程为空</p>
          <p className="text-xs text-gray-400 mt-1">模板生效但未设计事件</p>
        </div>
      ) : (
                  ...
                  {hourEvents.length === 0 ? (
                    ...
                    hourEvents.map((ev, i) => (
                      <motion.button
                        key={i}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => handleToggle(schedule.events.indexOf(ev))}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {ev.note && (
                            <p className="text-xs text-gray-400 truncate mt-0.5">{ev.note}</p>
                          )}
                        </div>
                      </motion.button>
                    ))
