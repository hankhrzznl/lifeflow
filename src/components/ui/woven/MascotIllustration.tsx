...
type MascotState = "idle" | "weaving" | "completed" | "confused" | "celebrating";
...
export default function MascotIllustration({
  ...
  return (
    <motion.div
      animate={state === "weaving" ? { y: [0, -3, 0] } : {}}
      transition={{ duration: 1.5, repeat: state === "weaving" ? Infinity : 0, ease: "easeInOut" }}
    ...
    >
      <svg width={s} height={s} viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <circle cx="60" cy="48" r="20" stroke={secondaryColor} strokeWidth="2" fill="none"
          strokeDasharray="4 2" opacity="0.7" />

        {/* ── 耳朵 ── */}
        <polygon points="40,30 34,12 48,25" fill={primaryColor} opacity="0.18" />
        <polygon points="40,30 34,12 48,25" stroke={secondaryColor} strokeWidth="2" fill="none"
          strokeDasharray="2 2" opacity="0.6" />
        <polygon points="80,30 86,12 72,25" fill={primaryColor} opacity="0.18" />
        <polygon points="80,30 86,12 72,25" stroke={secondaryColor} strokeWidth="2" fill="none"
          strokeDasharray="2 2" opacity="0.6" />
        ...
        {/* ── 毛线针手臂 ── */}
        <motion.g
          animate={state === "weaving" ? { rotate: [0, 5, -5, 0] } : {}}
          transition={{ duration: 1, repeat: state === "weaving" ? Infinity : 0 }}
        >
        ...
        {state === "weaving" && (
          <>
            <motion.path
              d="M38 60 Q30 75 45 78 Q55 80 50 65"
              stroke="#F5C542" strokeWidth="2" fill="none" strokeDasharray="3 2"
              animate={{ pathLength: [0.3, 0.7, 0.3] }}
              transition={{ duration: 2, repeat: Infinity }}
            />
            <motion.path
              d="M82 60 Q90 75 75 78 Q65 80 70 65"
              stroke="#E88D67" strokeWidth="2" fill="none" strokeDasharray="3 2"
              animate={{ pathLength: [0.7, 0.3, 0.7] }}
        ...
        {state === "completed" && (
          <motion.g initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }}>
            <rect x="48" y="28" width="24" height="16" rx="2"
              fill="#7CA982" opacity="0.3" stroke="#7CA982" strokeWidth="1.5"
              strokeDasharray="3 2" transform="rotate(-5 60 36)" />
            <text x="53" y="41" fontSize="8" fill="#7CA982" fontFamily="var(--font-display)" opacity="0.8">
              ✓
            </text>
          </motion.g>
        )}
        ...
        {state === "confused" && (
          <>
            ...
            <text x="62" y="74" fontSize="8" fill={secondaryColor} opacity="0.6" fontFamily="var(--font-display)">
              ?
            </text>
          </>
        )}
