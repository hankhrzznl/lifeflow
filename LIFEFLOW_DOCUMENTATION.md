# LifeFlow 产品与架构文档（合并版）

> 本文档为项目**唯一**产品/架构文档（含结构化快照 + 活约定流水账）：以**当前线上版本**（GitHub `main` 分支最新部署，https://www.my-lifeos.com/）为准，已消除历史文档之间的冲突。旧文档中的过时描述（旧路由、旧数据表、旧导航结构）不再作为现行事实，仅在有价值时以「历史参考」标注。
>
> 文档维护约定：后续变更直接更新本文档；新增功能落地后先在此登记约定（第十一章流水账），再同步结构化章节。

---

## 一、产品定位

**LifeFlow 是一款「目标驱动的个人生活操作系统」（Life OS），不是待办清单。**

- **解决什么问题**：市面上日程 App、习惯 App、记账 App 各管一摊、数据彼此孤立。LifeFlow 用统一本地数据层（IndexedDB/Dexie）串联「目标 → 计划 → 执行 → 记录 → 复盘」的完整闭环，让用户"一个地方看清时间的去向和生活的节奏"。
- **核心主张**：`捕捉 · 规划 · 专注 · 复盘`（见 `src/app/layout.tsx`）。
- **技术形态**：Next.js 全栈 PWA（430px 移动端容器、离线可用、可安装），数据完全本地存储；AI 助手（`src/app/assistant/page.tsx`）通过全局悬浮球自然语言录入。
- **发展主线**：已从「工具集合」演进为「理想日操作系统」（T19/T21），以 8+8+8 时间观 + 理想日蓝图 + 备考引擎为骨架。

### 目标用户

| 用户群体 | 使用场景 | 对应模块 |
|---|---|---|
| 备考学生（主画像） | 2027 安徽省考 + 2026 四级双线备考，倒排计划、每日学习块执行 | 备考计划、理想日蓝图、记忆复习、专注计时 |
| 自我提升者 | 定长期目标 → 五层拆解 → 每周任务 → 每日行动 | 目标（/efficiency-v2）、长期主义复盘 |
| 健身/健康人群 | 力量/有氧训练记录、体态拉伸、功法养生、饮水/睡眠/久坐提醒 | 训练中心、坐姿健康、睡眠、饮水 |
| 习惯养成者 | 习惯打卡、每日三件事、睡前仪式 | 习惯打卡、今日三件事、睡前仪式 |
| 记账理财用户 | 多账户收支、周/月复盘、每周一句话总结 | 记账、财务复盘 |

**关键场景**：每日打开首页驾驶舱 → 看今日执行 → 勾选完成 → 晚间睡前仪式 → 每周复盘回顾 → 月度回到目标校准。
...
| 数据库 | Dexie.js（IndexedDB 封装） | 4.4.2 |
| 动画 | Framer Motion | 12.38.0 |
| 图表 | Recharts | 3.9.2 |
| 图标 | Lucide React | 1.14.0 |
| 状态管理 | Zustand | 5.0.14 |
| Markdown | react-markdown + remark-gfm | 10.1.0 |
| PWA | Serwist（Service Worker + manifest） | 9.5.11 |
| 部署 | Vercel，域名 `https://www.my-lifeos.com/` | — |

**命令**：`npm run dev`（开发）/ `npm run build`（构建至 `.next/`）/ `npm start`（预览）/ `npm run lint`（检查）。
...
| 模块 | 路由 | 说明 |
|---|---|---|
| 首页 | `/` | 今日驾驶舱：今日待办 + 三件事、健康概览条、睡前仪式（21:00+）、今日一句话、快捷录入（记账/打卡/新建）；右上角日间/夜间全局主题切换 |
| 目标 | `/efficiency-v2` | 五层目标列表 + 今日焦点行 + 状态过滤网格 + 创建向导（5 步） |
| 日程 | `/efficiency/schedule` | 小时块时间轴、作息/训练/理想日事项、时间冲突检测、矫正 |
| 理想日 | `/ideal-day` | 8+8+8 时间轴模板、两步编辑向导、一页一段规划（L2） |

- **AI 助手**：全局悬浮球 → `/assistant`，自然语言录入事项/查询日程/生成复盘，不入底部导航。
- **长期主义**（`/longtermism`）：仅保留复盘时间轴（周/月 2 粒度）+ 历史复盘折叠区（T20 精简，金字塔模块卡已下线）；入口由首页「今日一句话」/复盘卡片承载。

### /more 功能目录（8+8+8 时间地图，T20）

实际结构为**四组 25 项**（`src/app/more/page.tsx` `MODULE_GROUPS`），卡片大小 = 模块占据时间的可视化映射（XL/L/M/S）：

| 分组 | 模块 |
|---|---|
| 计划与复盘 | 长期主义、复盘总览、目标拆解、理想日蓝图、习惯打卡、作息模板、课程表、备考目标、记忆复习、专注计时、冥想放松、周报汇总 |
| 身体养护 | 睡眠、饮水、训练中心、饮食、坐姿健康、吃药提醒（维修模式条件激活） |
| 生活 | 记账、备忘录、心愿、倒数日、日历（无目录入口，路由与数据保留） |
| 系统 | 提醒中心、设置 |
...
- **L1 功能层**：`/ideal-day` 5 大段时间轴（睡眠/上午/中午/下午/晚上）+ 功能图标
- **L2 规划层**：`/ideal-day/plan/[blockId]/[feature]`——独立页仅 study/sleep/medication；workout/wellness/posture/routine 为内嵌底部表单；其余跳模块页
- **L3 执行层**：日程页 `generateIdealDayItems` 自动消费

### 入口治理

- **单一入口原则**：同一功能只保留一个入口、一套数据、一条路由；旧版功能隐藏入口但保留路由与数据
- **路由治理**：`src/proxy.ts` 308 永久重定向（Next.js 16 起 `middleware.ts` 约定已废弃）；禁止链式重定向，旧路由直达新版
- 已重定向：`/more/ideal-day`→`/ideal-day`、`/efficiency*`→`/efficiency-v2`、`/tasks`、`/plugins/*`、`/focus`→`/more/focus` 等

### 复盘层级

首页驾驶舱（HomeReview，日粒度 Top4）→ 长期主义（周/月）→ `/more/review`（财务/日程模块复盘）+ `/more/review/weekly`（周报汇总）。

---

## 四、数据层

**本地优先**：数据全部存储于 IndexedDB（Dexie），无服务端；PWA 离线可用。共 **8 个 Dexie 实例**按域拆分：

| 库 | 文件（版本） | 核心表 |
|---|---|---|
| LifeFlowDB | `src/lib/db.ts`（v29） | userSettings（含理想日 plans/双作息/模板等）、reminders、goals/plans/tasks（AI 目标引擎）、customGoalTypes、correlationReports、finBudgets 等 |
| LifeFlowDaylog | `src/lib/db/daylog.db.ts`（v6） | items（日程事项，sourceType 语义）、courses、routineTemplates、routineTemplateGroups |
| LifeFlowHealth | `src/lib/db/health.db.ts`（v12） | waterLogs（唯一流水源）、waterGoals、sleepLogs、sleepGoals、workoutSessions、stretchLogs、trainingPlans、medicines、medicineLogs、postureSettings |
| LifeFlow | `src/lib/db/life.db.ts`（v8） | habits、focusSessions、meditationSessions、dietLogs、wellnessLogs、cheatDays、countdowns、notes、wishes |
| LifeFlowGoalV2 | `src/lib/db/goal-v2.db.ts`（v2） | goalV2Goals / goalV2KeyResults / goalV2Strategies / goalV2WeeklyTasks / goalV2DailyActions（五层拆解） |
| LifeFlowEfficiency | `src/lib/db/efficiency.db.ts`（v20） | goals（训练体系专用）、scheduleTasks（作息同步/日历活跃表） |
| LifeFlowEbbinghaus | `src/lib/db/ebbinghaus.db.ts`（v1） | decks、cards、reviewLogs |
| LifeFlowAccounting | `src/lib/db/accounting.db.ts`（v4） | ledgers、accounts、transactions、categories |

**localStorage 补充**（轻量/非结构数据）：三件事（`lifeflow_three_things`）、主题（`lf-theme`）、引导进度（`lifeflow_guide_progress`）、提醒默认（`lifeflow_reminder_defaults`）、睡前仪式、理想日执行意图等。

### 关键数据约定

- **sourceType 语义隔离**：日程 items 的 `sourceType` 为模块归属（routine/water/ideal/goal/habit/manual/course…），各模块互不污染；目标行动一律 `'goal'`，理想日独立 `'ideal'` 不写 GoalV2
- **数据裁决**：饮水 ml 一律读 `waterLogs`（唯一流水源）；GoalV2 日行动双向勾选同步日程 Item；作息走模板链（`generateRoutineItems`）；提醒存 LifeFlowDB.reminders 按 moduleType 关联
- **⚠️ Dexie 升级铁律**：版本号必须递增、保留旧 version 定义、新增字段只能新增 version（禁止改旧 stores）；删表必须显式 `表名: null`（否则 deleteRemovedTables 永不触发）；本地验证需干净重启 dev server
- **时间处理**：日期拼接禁止 `toISOString()`（东八区回退坑），统一 `getFullYear/getMonth/getDate`
...
- **今日待办单卡**：进度环（`--lifeflow-knit-bg` 底色）+ 今日三件事（可点击编辑，`updateText`）+ 执行流（`useTodayExecution` 单一数据源，与目标页焦点一致，前 8 条）+ 矫正提醒
- **健康概览条**：睡眠 / 饮水 / 习惯 / 专注 四格，点击跳对应模块
- **晚间睡前仪式**（21:00 后）：`SleepRitualCard` 渐进式早睡
- **今日一句话**：`UnifiedReviewer.generateReview("daily")` headline，跳 `/longtermism`
- **快捷录入**（悬浮组）：记一笔 / 打卡 / 新建事项（BottomSheet）
- **日间/夜间切换**：右上角月亮/太阳按钮全局切换浅色/深色主题

### 5.2 目标系统（GoalV2，`/efficiency-v2`）

**五层拆解**（`goal-v2.db`）：愿景 Goal → 关键结果 KR → 策略 Strategy → 周任务 WeeklyTask → 日行动 DailyAction。

- 进度 = 全部 KR 的 current/target 均值；无 KR 回退 Strategy→DailyAction 完成率聚合
- 4 种预置策略模板（fitness/skill/finance/career）
- 创建向导 5 步不可跳过；支持「复制提示词 / 导入计划」（sessionStorage `import_goal`）
- 详情页：KR 编辑（`-`/输入/`+` stepper、一键达标）、策略区（添加策略、进度小结）、今日理想日推进卡片
- 与理想日深度联动：study 规划绑定 goalId，双向展示

### 5.3 日程页（`/efficiency/schedule`）

- 小时块时间轴：默认只显示每块第一个事项，`+N 项 详情` 内联展开，可同时展开多块
- 事项卡片：勾选完成、长按删除、校准、备注、快捷操作；**历史事项（含已完成）点击打开备注**（T22.8 修复）
- 时间冲突检测：`conflict-detector.ts` 纯函数引擎，冲突项红色边框
- 理想日项紫色 `Sparkles + 理想日` 徽标；goal 项「专注」按钮带 `?title=&duration=` 预填

### 5.4 理想日操作系统（T19/T21/T22）

**8+8+8 基线**：睡眠 8h（22:30→06:00 夜间 7.5h + 12:30 午睡 0.5h）· 目标 8h（上午 3+下午 3+晚上 2）· 生活 8h。

**模板系统（T22.8）**：
- 模板选择优先级：① 手动锁定（`activeTemplateId` 存在）无条件优先，不检查日期范围 → ② 自动模式按使用日期范围（`startDate/endDate`）→ ③ daysOfWeek → ④ 列表首个
- `activeTemplateId` 无值 = 自动模式；「恢复自动」按钮清除锁定；模板管理区可设每模板起止日期
- 兼容：存量配置保留原值不迁移；切换/保存不删除已生成日程；不动双作息（暑假/开学）mode 机制

**双作息（T21-7）**：暑假/开学两套配置（`idealDaySummerConfig`/`idealDaySchoolConfig`）一键切换。

**规划形态（T22.2）**：12 功能三种形态——独立页（study/sleep/medication）/ 内嵌底部表单（workout/wellness/posture/routine，结构化清单）/ 删除或跳模块（water/diet/focus/notes/leisure）。

**执行引导（T19）**：块前 10 分钟提醒、自由时间执行意图、娱乐配额超时飘红。

### 5.5 备考引擎（T21）

- **省考**（2027.3.15，硬约束）：科目顺序学完制（判断→政治→申论→言语→资料→数量），4h/天，60 课时 127h，**9/16 前学完**
- **四级**（2026.12.12）：三阶段视频 102h（夯实基础→专项突破→提升能力+冲刺），2h/天，周一至五刷视频、周六日复习
- 倒排算法：剩余课时 ÷ 剩余天数；`LessonProgress` 追踪课时完成

### 5.6 长期主义 / 复盘（UnifiedReviewer）

- 统一引擎 `src/lib/brains/reviewer.ts`：`generateReview(period, offset)` 生成 headline/overview/模块洞察/行动建议
- 首页仅日粒度 Top4；长期主义周/月 + 历史复盘
- `/more/review`：财务（收支/分类/每周一句话）/ 日程模块复盘
- `/more/review/weekly`（T23.1）：近 7 天 6 项统计 + 专注柱状图 + 洞察文案 + 目标进度

### 5.7 训练中心（`/more/fitness`，T15b）

单入口三 Tab：**训练**（`workoutSessions`）/ **体态拉伸**（`stretchLogs`）/ **功法养生**（`wellnessLogs`）；`?tab=` 参数兼容旧深链；`/more/posture`、`/more/wellness` 重定向至此。

### 5.8 健康与习惯模块

- **睡眠**（`/more/sleep`）：入睡打卡、睡眠目标、统计（healthDB sleepLogs/sleepGoals）
- **饮水**（`/more/water`）：三时段目标制（T15，上午/下午/晚上占比 35/40/25，睡前 2h 不计入）、每次一条 waterLogs 流水、`/more/water` 历史记录 + 「生成饮水提醒」按钮
- **饮食**（`/more/diet`）：三餐记录（dietLogs）
- **坐姿健康**（`/more/posture-health`）：喝水 + 提肛 + 久坐休息合并单一功能（T21-5）
- **习惯打卡**（`/more/habits`）：按天打卡（habits.days[date]）、热力网格
- **今日三件事**（T21-4）：每日自动生成（省考/四级今日课时）+ 手动可调（点击 ✎ 编辑，编辑后转手动项）+ 完成联动备考进度
- **睡前仪式**（T21-3）：21:00 环境营造提醒 → 21:30 倒计时 → 22:00 入睡打卡；渐进式目标 23:30 起步逐周提前 30 分钟
- **吃药提醒**（维修模式）：仅在有用药计划或设置开启时全站浮现（medicines/medicineLogs）

### 5.9 效率与专注

- **专注计时**（`/more/focus`）：番茄专注（focusSessions）；理想日联动自动回写完成态
- **冥想放松**（`/more/meditation`，T23.1）：正念/呼吸/身体扫描/引导 4 方式，5-30 分钟，圆形计时环，写 meditationSessions
- **记忆复习**（`/more/ebbinghaus`）：艾宾浩斯曲线（0/1/2/4/7/15 天），decks/cards/reviewLogs
- **课程表**（`/more/schedule/courses`）、**作息模板**（`/more/schedule/routines`，含课堂节奏 45+5）

### 5.10 记账（`/more/accounting`）

多账本/多账户、收支记录、分类管理（LifeFlowAccounting 库）；财务复盘（周/月汇总、每周一句话总结）。

### 5.11 其他

- **备忘录 / 心愿 / 倒数日**（life.db）：随手记、心愿清单、重要日子倒数
- **日历**（`/more/calendar`）：月视图总览（目录无入口，数据保留）
- **周报汇总**（`/more/review/weekly`）：见 5.6

### 5.12 AI 助手（`/assistant`）

- 自然语言：记录事项（daylog items，`sourceType='goal'`/manual）、查询日程、生成复盘
- 动作已迁移至新版 API：习惯打卡→life.db Habit、目标→GoalV2 五层 API、饮水→waterLogs、`navigate_review`→`/more/review`

---

## 六、智能引擎层（`src/lib/brains/`）

| 模块 | 职责 |
|---|---|
| `assistant` | AI 对话助手核心 |
| `downgrade` | 降级策略引擎 |
| `guide` | 引导式任务推荐（Onboarding 进度，localStorage `lifeflow_guide_progress`） |
| `insight` | 数据洞察/分析 |
| `memory` | 长期行为模式挖掘 |
| `planner` | 目标自动拆解 |
| `presence` | 用户在线状态 |
| `reviewer` | UnifiedReviewer 复盘引擎（日/周/月/年能力，前端收敛日/周/月） |
| `scheduler` | 任务排序与时间槽优化 |

---

## 七、设计系统（Warm Gray + Blue）

**品牌蓝 `#2563EB`**；页面底 `#F8F8F7` 暖灰 + 白卡片分层。核心 CSS 变量（`src/app/globals.css`，均以 `var(--xxx)` 引用，禁硬编码）：

| 变量 | 亮色值 | 用途 |
|---|---|---|
| `--lifeflow-primary` | `#2563EB` | 品牌蓝（主按钮/激活态） |
| `--lifeflow-background` | `#F8F8F7` | 页面底色 |
| `--color-surface-card` | `#FFFFFF` | 卡片背景 |
| `--color-surface-secondary` | `#F0F0ED` | 次级表面 |
| `--color-text-primary` | `#1C1C1E` | 主文字 |
| `--color-text-secondary` | `#6B7280` | 次文字 |
| `--lifeflow-border` | `#E8E8EA` | 边框/分割线 |
| `--lifeflow-knit-bg` | `#E9E9EB` | 进度环底色 |
| `--color-expense` / `--color-income` | `#FF3B30` / `#34C759` | 支出红 / 收入绿 |
| `--shadow-card` | `0 1px 1px rgba(0,0,0,0.02), 0 2px 16px rgba(0,0,0,0.04)` | 卡片投影 |

- **主题**：`ThemeProvider`（light/dark/system，默认 light），日间/夜间切换按钮全局生效；深色模式为 class 策略（`@custom-variant dark`），所有 `dark:` 类跟随全局开关而非仅系统偏好
- **布局**：移动端 430px 容器；Bottom Sheet 操作按钮固定底部；多步向导用 `flex flex-col h-screen`（禁止 `fixed`）
- **图标**：lucide-react 统一
- 历史参考：早期存在「织光者的工作台」设计语言探索（编织隐喻/毛线橙 `#E88D67`/吉祥物小织），**未应用于当前线上版本**，见附录 D

---

## 八、关键规则与约定（铁律）

1. **数据不丢**：任何重构/删除仅限渲染层与入口，数据库表与数据不动；隐藏入口但保留路由与数据
2. **单一入口**：每类用户动作只有一条主路径；无价值页面不占任何入口
3. **Dexie 升级**：版本递增 + 保留旧定义 + 删表显式 `null`（见数据层）
4. **禁止空处理器**：交互元素 `onClick` 必须有可见状态变更或跳转（T22.8 经验教训）；新控件交付前做静态元素审计
5. **sourceType 隔离**：`'goal'`（目标）/ `'ideal'`（理想日）/ `'routine'`（作息）/ `'water'`（饮水）等互不混淆
6. **AI 提示词**：GoalV2 导入 JSON 中 `dailyActions.time/duration` 必填；`weeklyPattern` 键支持数字索引与英文星期名
7. **React Hooks 顺序**：404 提前 `return` 必须置于所有 hooks 之后（#310 教训）
8. **饮水口径**：ml 一律读 `waterLogs`；`daylog` water items 仅作待办展示

---

## 九、优化方向与路线图（未完待办）

以下条目源自已归档的 `LIFEFLOW_OPTIMIZATION_PLAN.md`，**仍待执行**：

- **T20-7**：理想日学习块 → 目标联动——点击学习块跳对应目标详情，目标删除时降级不报错
- **T20-8**：T20 全量验证 + 本文档（第九章/第十一章）登记 + git 提交

**持续有效的治理原则**：
- 目标主线：所有功能围绕「定目标 → 排计划 → 每日执行 → 记录习惯 → 复盘洞察」
- 优先级评分：用户价值 × 0.5 + 实现成本（低=高分）× 0.3 + 依赖度（少依赖=高分）× 0.2
- 行为科学：环境设计 > 意志力（睡前仪式、执行意图、娱乐配额数据反馈）
- 设计规范：复用既有组件、CSS 变量禁硬编码、iOS 字体栈、CTA「动词 + 对象」

**双作息待细化**：开学模板已预置（6:00 起、晚自习等），开学后按实际课表细化。

---

## 十、版本与演进主线

### 版本历史（v2.0.0，2026-07，源自已归档的 CHANGELOG.md）

- 四级目标拆解引擎（Goal→Milestone→WeeklyTask→DailyAtom）、全链路回算、4 种场景模板、打卡闭环、PDCA 周复盘 6 步、目标演化系统、AI 辅助洞察、跨目标冲突检测、定时提醒系统、完整统计库、PWA 支持、3 种桌面小组件、主题个性化、数据导出/导入、织光者的工作台设计语言
- 技术：Next.js 16 + React 19 + TS + Tailwind v4、Dexie、Framer Motion、Recharts、Serwist PWA、Zustand

> 注：v2.0 时代的「织光者的工作台」设计语言与四级拆解引擎为早期形态；当前线上版本已演进为五层 GoalV2 与 Warm Gray + Blue 设计体系（见第七章与 5.2 节）。

### 演进主线（T1→T23）

1. **T1-T4（P0）**：目标 tab 切换新版、/more 总入口、plugins 收敛、/more/review 占位清理
2. **T5-T8（P1）**：长期主义卡片、饮水双轨统一、健康库归档、efficiency 软冻结
3. **T9-T12（P2）**：v1→v2 目标迁移工具、新用户引导、/more 搜索、plugins 路由下线
4. **T13-T15（P3）**：数据全量归一（删表铁律）、目标深度集成、训练中心单入口、全系统清理（单一入口第一性原理审计）
5. **T16**：v1 目标系统退役（projects/phases 物理删除，goals/scheduleTasks 保留）
6. **T17**：目标级复盘增强（UnifiedReviewer）
7. **T18**：能量金字塔重构（主库 v34 死表清除）
8. **T19**：理想日系统（8+8+8 蓝图 + 执行引导 + 达成率复盘）
9. **T20**：8+8+8 信息架构（3-tab→4-tab 导航、复盘收敛、/more 时间地图、长期主义精简）
10. **T21**：痛点驱动重构（备考引擎、睡前仪式、今日三件事、坐姿健康、双作息）
11. **T22**：理想日操作系统（4-tab、8+8+8 时间轴、一页一段规划、双向打通、模板日期范围 T22.8）
12. **T23**：冥想模块 + 周报汇总
13. **T24**：画布-线上协作约定（v2 画布作设计稿库、线上保功能、双向取长、分批上线）
...
> 原 `LIFEFLOW_GUIDE.md`（v1.1 起持续更新的设计约定与演进记录），已并入本文档。开发前必读；新增功能落地后在此追加约定小节，再同步结构化章节。
>
> 工作流约定：用户发出需求 → 先读取本文档 → 引导式提问澄清模糊部分 → 整理成正式任务提示词 → **等待用户确认后才执行**；禁止跳过确认环节直接动手。
...
**入口**：仅通过 `/more` 目录「计划与复盘」组的「记忆复习」模块进入（v2.10+ T20 起，原「长期主义模块卡片」入口随金字塔卡片下线），底部导航栏和首页不新增入口
...
- 优化方向与路线图以本文档第九章为准（原 `LIFEFLOW_OPTIMIZATION_PLAN.md` 已归档删除）
- 重大架构变更/新模块引入前，应先在本章登记再实施，保持单一事实来源
...
### /more 目录 8+8+8 分组（v2.10+，T20；v2.14+ 修正为四组）— 时间地图替代能量金字塔

- **背景**：原 E1-E4 能量金字塔分组（能量底座/目标执行/过程记录/成长储备/维修/系统）与用户「8+8+8」时间观不符，整体重构为时间地图，卡片大小 = 模块占据时间的可视化映射（**时间地图**）
- **四组映射**（`src/app/more/page.tsx` `MODULE_GROUPS` 实际结构，25 项）：
  - **计划与复盘**：长期主义、复盘总览、目标拆解、理想日蓝图、习惯打卡、作息模板、课程表、备考目标、记忆复习、专注计时、冥想放松、周报汇总
  - **身体养护**：睡眠、饮水、训练中心、饮食、坐姿健康、吃药提醒（维修模式条件激活，条目级）
  - **生活**：记账、备忘录、心愿、倒数日、日历（日历无目录入口，路由与数据保留）
  - **系统**：提醒中心、设置
...
- **勿回归**：勿将金字塔分组/长期主义模块卡片重新引入；新增模块按语义放入对应组
...
- **内部链接一致性**：专注页唯一正式地址为 `/more/focus`（旧 `/focus` 亦 308 至该页）；全站入口（TaskDetail、AgentProvider、more/projects）已统一指向 `/more/focus`
...
- **入口**：`/ideal-day` 配置页（原 `/more/ideal-day` 308 至此），本文档约束单一入口、430px 容器
...
- **达成率复盘**（`src/lib/brains/reviewer.ts` `_analyzeIdeal`）：四维 = 睡眠（夜间按时 + 午睡完成）/ 学习（ideal-study 块完成分钟）/ 饮水（waterLogs≥蓝图 waterTargetMl）/ 配额（leisure 块完成率），四维平均得整体达成率 + 薄弱维度发现；module=`ideal`（色 #FF2D55，icon Sparkles），首页 MODULE_ROUTES → `/ideal-day`，长期主义随 insights 自动展示
...
> 核心转向：**不是拿现有功能凑方案，而是从痛点出发重新设计功能**（第一性原理）。方案已登记本文档第九章，本节约定在实施前生效。
...
### 理想日模板选择优先级 + 使用日期范围（T22.8）

> 根因：模板切换条芯片 `onClick` 曾误接 `setEditMode(false)` 空操作，点击「工作日 5 天」等模板无任何反应；经用户裁决确定「手动激活无条件优先」与「自动模式+恢复入口」的共存机制。

- **模板选择优先级**（`selectTemplate`）：① `activeTemplateId` 存在 → **无条件返回该模板（手动锁定，不检查日期范围/星期）**；② 无值（自动模式）→ 找 `startDate/endDate` 覆盖当天的模板（可单边留空=不限）；③ 其次 `daysOfWeek` 匹配；④ 兜底返回列表首个
- **手动锁定**：点击模板芯片 = 写 `activeTemplateId` 并 `saveIdealDayConfig` 持久化；锁定模板芯片显示锁图标；非编辑态模板管理区显示「已手动锁定」指示
- **自动模式**：`activeTemplateId` 无值即自动；「恢复自动」按钮清除 `activeTemplateId` 回自动模式（不删模板与数据）
- **数据模型**：`IdealDayTemplate` 新增可选 `startDate/endDate`（YYYY-MM-DD，单边留空=不限）；模板管理区（非编辑向导）提供每模板起止日期输入
- **兼容**：`ensureTemplates` **不再强制填充默认 `activeTemplateId`**（存量配置保留原值不迁移，指向不存在模板时视为自动模式）；切换/保存不删除已生成日程；不动双作息（暑假/开学）mode 机制

### 交互元素审计（经验教训，T22.8 根因层）

> 本质：交互元素必须「所见即所点」——有可点击外观就必有有效行为。

- **禁止空处理器**：`onClick` 必须产生可见状态变更或跳转；严禁接无意义操作（本次根因：芯片 `onClick={() => setEditMode(false)}`，非编辑态下为空操作）
- **静态元素审计**：新页面/新控件交付前排查「有视觉可点击外观（cursor-pointer/按压反馈/可点文案）但无有效行为」的元素；发现即缺陷，修复或明确为展示态
- **死逻辑清理**：形如 `locked || f === 'sleep' && locked` 的恒失效条件（等价于 `locked`）及时清理，避免误导后续维护

### 画布-线上协作约定（v2.15+，T24）— 设计稿与生产职责分离

> 本质：**「设计稿」与「生产实现」是两类工件，职责分离才能保证可上线性**——设计稿负责视觉/交互方向，生产代码负责数据与逻辑；互相污染会导致静态稿侵入生产（教训：记账页升级时曾误删右上角「账户」入口，已回滚保留）。

- **画布定位**：`lifeflow-home-redesign-v2`（33 页）为设计稿库，视觉/交互方向基准；线上 `src/app` 为生产功能基准，数据权威始终在线上（Dexie 表、sourceType 语义、单一入口治理）
- **双向取长原则**：
  - 画布页按线上功能逻辑重做（模拟 CRUD/持久化语义，如 `expense/training/posture/wellness/courses` 五页）；画布作高保真可交互设计稿
  - 线上页按画布视觉升级，且**功能逻辑逐行保留**；验收：`tsc --noEmit` + `build` 通过、lint 净新增为 0（基准对照）、不新建路由、不新建/删 Dexie 表
- **画布有而线上无独立路由的页面**（成就中心/数据洞察/导出等）：不新建路由，价值并入现有模块（周报数据洞察、睡眠热力图、设置内导出）
...
以下内容来自旧模块文档，对应路由/数据表已下线，**仅作设计参考**：

- **旧助手子模块**（`/assistant/{finance,fitness,sleep,water}`）：已并入独立模块页（/more/accounting、/more/fitness、/more/sleep、/more/water）。可保留参考：记账 12 支出 + 6 收入分类口径、月度结余 = 初始余额 + 收入 − 支出；健康评分「饮水 25 分/2000ml」；RPE 训练记录字段设计
- **旧捕捉/规划/项目体系**（`/planner`、`/pending`、`/goals`、`/projects`、Project→Board→Section→Task 四级）：已由目标 V2 + 日程 items 取代；四象限优先级定义仍可参考
- **旧复盘页**（`/review` PDCA）：已由 UnifiedReviewer 取代；「亮点/问题/改进点」三段结构与闭环行动理念延续至今
- **旧 v1 目标**（efficiencyDB projects/phases）：已物理删除；`goals`/`scheduleTasks` 保留供训练计划生成器/作息同步/日历依赖

## 附录 B：仓库内其他 .md 文件说明（合并来源）

| 文件 | 处理 |
|---|---|
| `AGENTS.md` | 工作区/agent 规则，非产品文档，**保留独立** |
| `README.md` | 产品级 README，**保留独立** |
| `LIFEFLOW_GUIDE.md` | 设计约定与规则流水账，已并入第十一章，**已删除** |
| `LIFEFLOW_OPTIMIZATION_PLAN.md` | 演进史/路线图，要点已并入第九、十章（含待办 T20-7/T20-8），**已删除** |
| `CLAUDE.md` | 空文件，无内容，**已删除** |
| `CHANGELOG.md` | v2.0 历史，已并入第十章，**已删除** |
| `LIFEFLOW_PROJECT_INTRO.md` / `LIFEFLOW_FULL_DOCUMENTATION.md` / `LIFEFLOW_COMPLETE_DOCUMENTATION.md` | 旧架构快照（内容已被本文档以当前状态取代），**已删除** |
| `ASSISTANT_*_MODULE.md` / `CAPTURE_PLANNER_MODULES.md` / `REVIEW_MODULE.md` | 已退役模块文档，要点已并入附录 A，**已删除** |
| `lifeflow-woven-workbench/` 目录（含全部 json/html） | 未上线设计系统探索（毛线橙编织风），与线上 Warm Gray + Blue 冲突，整体清理，**已删除** |
| `lifeflow-design-guide/`、`lifeflow-launch-audit/`、`lifeflow-roadmap/`、`dist/` | 旧版设计规范/审查/路线图/Vite 构建产物，与当前版本无关，**已删除** |
| `postcss.config.cjs`、`src/lib/*.bak`、`src/app/tasks/`、`src/app/more/ideal-day/` | 冲突配置/备份/死路由页面，**已删除** |

## 附录 C：织光者的工作台（历史参考，已删除）

早期曾存在 `lifeflow-woven-workbench/` 目录：一套**未上线**的设计系统探索——温暖大地色系（毛线橙 `#E88D67` 为主色）、编织/针织隐喻文案（"织完这一针"）、9 个组件 JSON 契约 + 预览 HTML、吉祥物「小织」。当前线上版本未采用此体系（现行设计为 Warm Gray + Blue）。该目录已随精简清理整体删除，本节仅作历史记录。

## 附录 D：合并原则与冲突消解记录

- **权威基准**：当前代码 + GitHub `main` 最新部署为准；文档与代码冲突时以代码为准
- **消解冲突示例**：底部导航 5-tab（旧文档）→ 4-tab（现行）；`/more` 六组 → 四组；6 库 26 表 → 8 库实例；每日循环饮水提醒 → 三时段目标制；媒体查询深色 → class 策略全局开关；四级拆解引擎 → 五层 GoalV2
- **未来维护**：本文档为唯一产品文档；新功能落地后，先在第十一章追加约定，再同步结构化章节
