    ...
    <div className="min-h-screen max-w-[430px] mx-auto px-4 pt-6 pb-[100px]">
      <header className="flex items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[17px] font-semibold" style={{ color: "var(--lifeflow-foreground)" }}>目标复盘</h1>
        </div>
      </header>
      <div className="rounded-[20px] p-8 text-center" style={{ background: "var(--color-surface-card)", boxShadow: "var(--shadow-card)" }}>
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4" style={{ background: "#6366F115" }}>
          <CheckCircle2 className="w-8 h-8" style={{ color: "#6366F1" }} />
        </div>
        <p className="text-[15px] font-semibold mb-2" style={{ color: "var(--color-text-primary)" }}>目标完成趋势</p>
        <p className="text-[13px]" style={{ color: "var(--color-text-secondary)" }}>详细的目标完成率图表即将上线。<br />先通过 AI 助手说"帮我复盘这周"查看文字摘要吧。</p>
      </div>
    </div>
  );
}
